...
import atlasBatch2Data from '@/fixtures/atlas_batch2.json';

export const importAtlasBatch2 = async () => {
  try {
    console.log('Starting Atlas Batch 2 import...');
    
    // First, upsert SNL catalog additions
    console.log('Upserting SNL catalog additions...');
    for (const snlNode of atlasBatch2Data.snl_catalog_additions) {
    ...
    console.log(`Upserted ${atlasBatch2Data.snl_catalog_additions.length} SNL nodes`);
    
    let importedCount = 0;
    let skippedCount = 0;
    
    for (const loopData of atlasBatch2Data.loops) {
    ...
    console.log(`Atlas Batch 2 import completed! Imported: ${importedCount}, Skipped: ${skippedCount}`);
    
    toast({
      title: "Atlas Batch 2 Import Complete",
    ...
    console.error('Atlas Batch 2 import failed:', error);
    toast({
      title: "Import Failed",
