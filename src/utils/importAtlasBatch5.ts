...
import atlasBatch5Data from '@/fixtures/atlas_batch5.json';

export const importAtlasBatch5 = async () => {
  try {
    console.log('Starting Atlas Batch 5 import...');
    
    // First, upsert SNL catalog additions
    console.log('Upserting SNL catalog additions...');
    for (const snlNode of atlasBatch5Data.snl_catalog_additions) {
    ...
    console.log(`Upserted ${atlasBatch5Data.snl_catalog_additions.length} SNL nodes`);
    
    let importedCount = 0;
    let skippedCount = 0;
    
    for (const loopData of atlasBatch5Data.loops) {
    ...
    console.log(`Atlas Batch 5 import completed! Imported: ${importedCount}, Skipped: ${skippedCount}`);
    
    toast({
      title: "Atlas Batch 5 Import Complete - NCF-PAGS Atlas Complete!",
      description: `Successfully imported ${importedCount} loops (${skippedCount} skipped as duplicates). All 42 loops now available!`
    });
    
    return { success: true, imported: importedCount, skipped: skippedCount };
    
  } catch (error) {
    console.error('Atlas Batch 5 import failed:', error);
    toast({
      title: "Import Failed",
