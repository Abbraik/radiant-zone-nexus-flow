...
import atlasBatch4Data from '@/fixtures/atlas_batch4.json';

export const importAtlasBatch4 = async () => {
  try {
    console.log('Starting Atlas Batch 4 import...');
    
    // First, upsert SNL catalog additions
    console.log('Upserting SNL catalog additions...');
    for (const snlNode of atlasBatch4Data.snl_catalog_additions) {
    ...
    console.log(`Upserted ${atlasBatch4Data.snl_catalog_additions.length} SNL nodes`);
    
    let importedCount = 0;
    let skippedCount = 0;
    
    for (const loopData of atlasBatch4Data.loops) {
    ...
    console.log(`Atlas Batch 4 import completed! Imported: ${importedCount}, Skipped: ${skippedCount}`);
    
    toast({
      title: "Atlas Batch 4 Import Complete",
    ...
    console.error('Atlas Batch 4 import failed:', error);
    toast({
      title: "Import Failed",
