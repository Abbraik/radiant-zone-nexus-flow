...
export function runPDFMockTrajectory(scenario: { id: string; lps: { lpId: string; targetId: string; targetType: 'loop'|'bundle'|'policy' }[] }, months = 36){
  ...
  for (let t = 0; t <= months; t++){
    ...
    ;(Object.keys(state) as (keyof typeof state)[]).forEach((key)=>{
      ...
      const elasticity = lpEffects.length ? Math.max(...lpEffects.map(e=>(e as any).elasticity as number)) : 0.05
