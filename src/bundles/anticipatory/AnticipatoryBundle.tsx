  ...
  return (
    <div className="space-y-6">
      ...
      <div className="flex items-center justify-between">
        <div>
          <div className="text-sm text-muted-foreground">Anticipatory · Checkpoint</div>
      ...
      <div className="grid grid-cols-1 lg:grid-cols-2 gap-6">
        ...
        <div className="rounded-2xl border p-3">
          <div className="font-medium mb-2">Risk Watchboard</div>
          <div className="grid gap-3">
            {watchboard.length === 0 ? (
              <div className="text-sm text-muted-foreground text-center py-4">No risk channels yet</div>
            ) : (
        ...
        {/* Scenario Simulator */}
        <div className="rounded-2xl border p-3">
          <div className="font-medium mb-2">Scenario & Stress Simulator</div>
          <div className="text-sm text-muted-foreground mb-2">Run a scenario, compare with/without mitigation, then plan tasks.</div>
        ...
        {/* Pre-Positioner */}
        <div className="rounded-2xl border p-3">
          <div className="font-medium mb-2">Pre-Positioner</div>
          <div className="text-sm text-muted-foreground mb-2">Stage resources, regulatory, and comms packs (quiet, no public activation).</div>
          <div className="grid gap-3 mb-3">
            {prePositionPacks.length === 0 ? (
              <div className="text-sm text-muted-foreground text-center py-4">No pre-position packs available</div>
            ) : (
          ...
          <button
            className="px-3 py-1 rounded bg-accent text-accent-foreground text-sm disabled:opacity-50 focus:ring-2 focus:ring-ring w-full"
        ...
        {/* Trigger Library */}
        <div className="rounded-2xl border p-3">
          <div className="font-medium mb-2">Trigger Library</div>
          <div className="text-sm text-muted-foreground mb-2">Guardable IF-THEN rules to auto-activate pre-positioned bundles.</div>
          <div className="grid gap-3">
            {triggerTemplates.length === 0 ? (
              <div className="text-sm text-muted-foreground text-center py-4">No trigger templates available</div>
            ) : (
      ...
      <div className="rounded-2xl border p-3 flex gap-2 flex-wrap">
        <div className="font-medium w-full mb-2">Handoffs</div>
        <button 
          className="px-3 py-1 rounded border text-sm disabled:opacity-50 focus:ring-2 focus:ring-ring"
