...
import { ResponsiveHeaderLanguage } from "./ResponsiveLanguage";
...
export default function ResponsiveBundle(props: ResponsiveBundleProps) {
  ...
  // Language mode state - persisted locally
  const [langMode, setLangMode] = React.useState<LangMode>(
    (localStorage.getItem("ui.langMode") as LangMode) ?? "general"
  );
  React.useEffect(() => { 
    localStorage.setItem("ui.langMode", langMode); 
  }, [langMode]);
    ...
    <div className="grid gap-4">
      ...
      <div className="flex items-start justify-between">
        <div>
          ...
          <div className="mt-1 text-sm">{indicator}</div>
        ...
        <div className="flex items-center gap-3">
          ...
          <ResponsiveHeaderLanguage mode={langMode} onModeChange={setLangMode} />
        </div>
      </div>

      {/* Guardrails */}
      <div className="rounded-2xl border p-3">
        <div className="font-medium mb-1">{label(responsiveDict, "GUARDRAIL", langMode)}</div>
