...
import { getDeliberativeLearningContent } from './learningHub.content';

interface DeliberativeHeaderLanguageProps {
  mode: LangMode;
  onModeChange: (mode: LangMode) => void;
}

export const DeliberativeHeaderLanguage: React.FC<DeliberativeHeaderLanguageProps> = ({ mode, onModeChange }) => {
  const [showLearningHub, setShowLearningHub] = useState(false);
  const learningContent = getDeliberativeLearningContent(mode);
  ...
  return (
    <div className="flex items-center gap-3">
      ...
      <Badge variant="secondary" className="text-xs">
        {mode === 'general' ? 'Group Decision' : 'Deliberative'}
      </Badge>
    </div>
  );
};