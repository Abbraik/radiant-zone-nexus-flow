-- Production-Safe Schema Hardening + FE Compatibility (Data-Safe)
...
--------------------------------------
-- E. Clean orphaned data before FKs
--------------------------------------
DO $$
DECLARE
    orphaned_count INTEGER;
BEGIN
    -- Clean orphaned tasks_5c records (if table exists)
    IF EXISTS (SELECT 1 FROM information_schema.tables WHERE table_name='tasks_5c') THEN
        -- Count and log orphaned records
        SELECT COUNT(*) INTO orphaned_count
        FROM public.tasks_5c t
        WHERE t.loop_id IS NOT NULL 
          AND NOT EXISTS (SELECT 1 FROM public.loops l WHERE l.id = t.loop_id);
        
        -- Delete orphaned records
        IF orphaned_count > 0 THEN
            DELETE FROM public.tasks_5c t
            WHERE t.loop_id IS NOT NULL 
              AND NOT EXISTS (SELECT 1 FROM public.loops l WHERE l.id = t.loop_id);
            
            RAISE NOTICE 'Cleaned % orphaned tasks_5c records', orphaned_count;
        END IF;
    END IF;

    -- Clean orphaned tasks_v2 records (if table exists)
    IF EXISTS (SELECT 1 FROM information_schema.tables WHERE table_name='tasks_v2') THEN
        ...
        IF orphaned_count > 0 THEN
            DELETE FROM public.tasks_v2 t
            WHERE t.loop_id IS NOT NULL 
              AND NOT EXISTS (SELECT 1 FROM public.loops l WHERE l.id = t.loop_id);
            
            RAISE NOTICE 'Cleaned % orphaned tasks_v2 records', orphaned_count;
        END IF;
    END IF;

    -- Clean orphaned incidents records (if table exists)
    IF EXISTS (SELECT 1 FROM information_schema.tables WHERE table_name='incidents') THEN
        SELECT COUNT(*) INTO orphaned_count
        FROM public.incidents i
        WHERE i.loop_code IS NOT NULL 
          AND NOT EXISTS (SELECT 1 FROM public.loops l WHERE l.loop_code = i.loop_code);
        ...
        IF orphaned_count > 0 THEN
            ...
            RAISE NOTICE 'Cleaned % orphaned incidents records', orphaned_count;
        END IF;
    END IF;
END$$;

--------------------------------------
-- F. Add key FKs (after cleanup)
--------------------------------------
DO $$
BEGIN
...
--------------------------------------
-- H. Compatibility Views (conditionally)
--------------------------------------
DO $$
BEGIN
...
--------------------------------------
-- I. Performance indexes (conditional)
--------------------------------------
DO $$
BEGIN
