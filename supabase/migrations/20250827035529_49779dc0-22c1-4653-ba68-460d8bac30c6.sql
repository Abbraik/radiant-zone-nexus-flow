-- Production-Safe Schema Hardening + FE Compatibility
...
--------------------------------------
-- G. Compatibility Views (no FE break)
--------------------------------------
-- v_loop_shared_nodes (for get_loop_hydrate and SNL joins)
CREATE OR REPLACE VIEW public.v_loop_shared_nodes AS
...
-- Optional: unify tasks into a read-only inbox (keep existing screens happy)
CREATE OR REPLACE VIEW public.v_tasks_inbox AS
SELECT
  t.task_id         AS id,
  t.loop_id,
  t.capacity,
...
WHERE EXISTS (SELECT 1 FROM information_schema.tables WHERE table_name='tasks_v2');

-- Fast latest normalized observation per (loop,indicator)
CREATE OR REPLACE VIEW public.v_indicator_latest AS
...
WHERE EXISTS (SELECT 1 FROM information_schema.tables WHERE table_name='normalized_observations');
