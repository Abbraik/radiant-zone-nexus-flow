-- Fix get_loop_hydrate function to handle missing indicators.loop_id column
DROP FUNCTION IF EXISTS public.get_loop_hydrate(uuid);
DROP FUNCTION IF EXISTS public.get_loop_hydrate(text);

-- Create the updated get_loop_hydrate function without indicators section
...
DECLARE 
  v_result jsonb;
BEGIN
  -- Build comprehensive result by loop ID (without indicators for now)
