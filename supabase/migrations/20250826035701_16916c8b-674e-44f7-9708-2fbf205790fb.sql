-- Seed Shared Nodes (SNL) Backbone and Golden-Path Loops - Fixed
...
BEGIN
    ...
    INSERT INTO loop_nodes (loop_id, label, kind, meta) VALUES
    (fertility_loop_id, 'Total Fertility Rate', 'stock', '{"domain": "population"}'),
    (fertility_loop_id, 'Desired Family Size', 'aux', '{"domain": "social"}'),
    (fertility_loop_id, 'Economic Pressure', 'aux', '{"domain": "resource"}'),
    (fertility_loop_id, 'Childcare Availability', 'stock', '{"domain": "institution"}'),
    (fertility_loop_id, 'Career Opportunity Cost', 'aux', '{"domain": "resource"}'),
    (fertility_loop_id, 'Social Support', 'aux', '{"domain": "social"}');

    -- Add edges for MAC-L01 (using subqueries to get proper node IDs)
    INSERT INTO loop_edges (loop_id, from_node, to_node, polarity, weight) 
    SELECT fertility_loop_id, 
           (SELECT id FROM loop_nodes WHERE loop_id = fertility_loop_id AND label = 'Desired Family Size'),
           (SELECT id FROM loop_nodes WHERE loop_id = fertility_loop_id AND label = 'Total Fertility Rate'),
           1, 0.7;

    INSERT INTO loop_edges (loop_id, from_node, to_node, polarity, weight)
    SELECT fertility_loop_id,
           (SELECT id FROM loop_nodes WHERE loop_id = fertility_loop_id AND label = 'Economic Pressure'),
           (SELECT id FROM loop_nodes WHERE loop_id = fertility_loop_id AND label = 'Total Fertility Rate'),
           -1, 0.8;

    INSERT INTO loop_edges (loop_id, from_node, to_node, polarity, weight)
    SELECT fertility_loop_id,
           (SELECT id FROM loop_nodes WHERE loop_id = fertility_loop_id AND label = 'Childcare Availability'),
           (SELECT id FROM loop_nodes WHERE loop_id = fertility_loop_id AND label = 'Total Fertility Rate'),
           1, 0.6;
    ...
    INSERT INTO loop_nodes (loop_id, label, kind, meta) VALUES
    (childcare_loop_id, 'Childcare Wait Times', 'stock', '{"domain": "institution"}'),
    (childcare_loop_id, 'Service Capacity', 'stock', '{"domain": "institution"}'),
    (childcare_loop_id, 'Quality Rating', 'aux', '{"domain": "social"}'),
    (childcare_loop_id, 'Funding Per Child', 'stock', '{"domain": "resource"}'),
    (childcare_loop_id, 'Staff Retention', 'aux', '{"domain": "institution"}'),
    (childcare_loop_id, 'Parent Satisfaction', 'aux', '{"domain": "social"}');
    ...
    -- LABOUR MARKET VERTICAL (MAC-L04)
    ...
    INSERT INTO loop_nodes (loop_id, label, kind, meta) VALUES
    (labor_mkt_loop_id, 'Vacancy-Unemployment Gap', 'stock', '{"domain": "resource"}'),
    (labor_mkt_loop_id, 'Skills Mismatch', 'aux', '{"domain": "resource"}'),
    (labor_mkt_loop_id, 'Time to Fill', 'stock', '{"domain": "institution"}'),
    (labor_mkt_loop_id, 'Search Efficiency', 'aux', '{"domain": "institution"}'),
    (labor_mkt_loop_id, 'Wage Flexibility', 'aux', '{"domain": "resource"}'),
    (labor_mkt_loop_id, 'Geographic Mobility', 'aux', '{"domain": "social"}');
    ...
    INSERT INTO loop_nodes (loop_id, label, kind, meta) VALUES
    (service_rel_loop_id, 'Service Outage Rate', 'stock', '{"domain": "institution"}'),
    (service_rel_loop_id, 'Maintenance Investment', 'stock', '{"domain": "resource"}'),
    (service_rel_loop_id, 'System Resilience', 'aux', '{"domain": "institution"}'),
    (service_rel_loop_id, 'User Complaints', 'flow', '{"domain": "social"}'),
    (service_rel_loop_id, 'Staff Morale', 'aux', '{"domain": "social"}'),
    (service_rel_loop_id, 'Budget Pressure', 'aux', '{"domain": "resource"}');
    ...
    INSERT INTO loop_nodes (loop_id, label, kind, meta) VALUES
    (trust_loop_id, 'Trust Index', 'stock', '{"domain": "social"}'),
    (trust_loop_id, 'Civic Engagement', 'stock', '{"domain": "social"}'),
    (trust_loop_id, 'Information Quality', 'aux', '{"domain": "institution"}'),
    (trust_loop_id, 'Community Events', 'flow', '{"domain": "social"}'),
    (trust_loop_id, 'Social Capital', 'stock', '{"domain": "social"}'),
    (trust_loop_id, 'Institutional Performance', 'aux', '{"domain": "institution"}');
