-- Extend loop_scorecards table with new columns
...
CREATE MATERIALIZED VIEW IF NOT EXISTS public.mv_loop_metrics AS
SELECT 
  l.id as loop_id,
  l.name as loop_name,
  ...
  -- Claim velocity (closed claims per week)
...
LEFT JOIN (
  -- Claim velocity (claims closed per week)
  SELECT 
    loop_id,
    COUNT(*) FILTER (WHERE status = 'completed' AND updated_at >= now() - interval '7 days') as velocity
  FROM public.claims
  GROUP BY loop_id
...
CREATE UNIQUE INDEX IF NOT EXISTS idx_mv_loop_metrics_loop_id ON public.mv_loop_metrics (loop_id);
...
-- Create RPC function to create redesign tasks
