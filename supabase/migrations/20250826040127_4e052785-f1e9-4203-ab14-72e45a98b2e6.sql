-- Signals Layer Database Schema (Fixed - window is reserved keyword)
...
CREATE TABLE IF NOT EXISTS public.loop_signal_scores (
    loop_id uuid NOT NULL REFERENCES public.loops(id) ON DELETE CASCADE,
    time_window text NOT NULL, -- '14d', '28d', etc.
    ...
    created_at timestamp with time zone NOT NULL DEFAULT now(),
    user_id uuid NOT NULL DEFAULT auth.uid(),
    PRIMARY KEY (loop_id, time_window, as_of)
);
...
CREATE INDEX IF NOT EXISTS idx_signal_scores_loop_window ON public.loop_signal_scores(loop_id, time_window);
