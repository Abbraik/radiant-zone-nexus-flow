-- Production Guardrails & Harmonization Schema
...
-- Hub allocations for shared resources
...
CREATE INDEX IF NOT EXISTS idx_hub_allocations_window ON hub_allocations USING gist(window);
...
WHERE ha.window @> now()::timestamp with time zone
GROUP BY ha.snl_id, ha.window, ha.region, lss.hub_load;
...
CREATE TRIGGER guardrail_policies_updated_at BEFORE UPDATE ON guardrail_policies
  FOR EACH ROW EXECUTE FUNCTION touch_updated_at();

CREATE TRIGGER hub_allocations_updated_at BEFORE UPDATE ON hub_allocations
  FOR EACH ROW EXECUTE FUNCTION touch_updated_at();
